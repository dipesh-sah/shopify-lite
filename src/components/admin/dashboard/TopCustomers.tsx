...
interface TopCustomersProps {
  customers: { id?: string; email: string; name: string; orders: number; totalSpent: number }[]
}
  ...
  return (
    <div className="rounded-xl border bg-card shadow-sm h-full flex flex-col">
      <div className="p-6 flex items-center justify-between border-b">
        ...
        <Link href="/admin/customers" className="text-sm text-primary group-hover:underline">View All</Link>
      </div>
        ...
        {customers.length === 0 ? (
          ...
          <div className="divide-y">
            {customers.map((customer, idx) => {
              ...
              return customer.id ? (
                <Link key={idx} href={`/admin/customers/${customer.id}`} className="block transition-colors">
                  <Content />
                </Link>
              ) : (
                <div key={idx}>
