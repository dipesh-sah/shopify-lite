...
interface ProductDetailClientProps {
  product: any
  initialReviews: any[]
  initialStats: { count: number; average: number }
}
                  ...
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        ...
                        className={cn(
                          "h-4 w-4",
                          i < Math.round(reviewStats.average) ? "fill-current text-yellow-500" : "fill-none text-gray-300"
                        )}
                      />
                    ))}
                  </div>
                  <span className="text-muted-foreground ml-1 text-slate-500">({reviewStats.count} reviews)</span>
                </div>
              </div>
