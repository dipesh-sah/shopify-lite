
import dotenv from 'dotenv';
import path from 'path';

// Load env from current directory
dotenv.config();

async function updateOrdersTable() {
  try {
    const { execute, query } = await import('../lib/db');

    console.log('Checking orders table schema...');

    // Check if columns exist
    const columns = await query("SHOW COLUMNS FROM orders LIKE 'shipping_first_name'");
    if (columns.length > 0) {
      console.log('Address columns already exist.');
      process.exit(0);
    }

    console.log('Adding address columns to orders table...');

    const alterSql = `
      ALTER TABLE orders
      ADD COLUMN shipping_first_name VARCHAR(255),
      ADD COLUMN shipping_last_name VARCHAR(255),
      ADD COLUMN shipping_company VARCHAR(255),
      ADD COLUMN shipping_address1 VARCHAR(255),
      ADD COLUMN shipping_address2 VARCHAR(255),
      ADD COLUMN shipping_city VARCHAR(255),
      ADD COLUMN shipping_province VARCHAR(255),
      ADD COLUMN shipping_zip VARCHAR(20),
      ADD COLUMN shipping_country VARCHAR(255),
      ADD COLUMN shipping_phone VARCHAR(50)
    `;

    await execute(alterSql);
    console.log('Orders table updated successfully.');

    process.exit(0);
  } catch (error) {
    console.error('Error updating orders table:', error);
    process.exit(1);
  }
}

updateOrdersTable();
